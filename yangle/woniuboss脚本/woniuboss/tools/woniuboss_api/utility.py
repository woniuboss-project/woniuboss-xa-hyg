...
class Utility:
    ...
    def get_excel_to_dict(cls, xls_file_info):
        ...
        for i in range(xls_file_info['STARTROW'], xls_file_info['ENDROW']):
            ...
            info = {'URL': url, 'METHOD': method, 'DATA': d, 'CODE': status_code, 'CONTENT': resp_content}
            test.append(info)
    ...
    def get_excel_to_user(cls, xls_file_info):
        ...
        for i in range(xls_file_info['STARTROW'], xls_file_info['ENDROW']):
            # 读取用户权限中的内容
            information = contents.cell(i, xls_file_info['INFORMATIONCOL']).value
            # 获取的是列表
    ...
    @classmethod
    def get_user_to_dict(cls, xls_file_info):
        # 将excel文件读取到内存中
        workbook = xlrd.open_workbook(xls_file_info['DATAPATH'])
        # 可以根据sheet页的下标或者名称读取sheet页中的内容.下标从0开始
        contents = workbook.sheet_by_name(xls_file_info['SHEETNAME'])
        ...
        for i in range(xls_file_info['STARTROW'], xls_file_info['ENDROW']):
            ...
            for t in temp:
                ...
                d[t.split('=')[0]] = t.split('=')[1]
            test_info.append(d)
        test_new_info = test_info
        # 将结果返回
        return test_new_info

    @classmethod
    def get_user_to_tuple(cls,xls_file_info):
        result = cls.get_user_to_dict(xls_file_info)
        li = []
        for i in result:
            tup = tuple(i.values())
            li.append(tup)
