...
class TestTodayAttendance(unittest.TestCase):
    ...
    def test_doone_attendance(self,url,post,data,status_code,expect):
        one_data=data['value']
        str_id,str_status=one_data.split('_')
        sql = f'select result from attendance where attendance_student_id={str_id} and create_time like "{local_time}%"'
    ...
    def test_search_student(self,url,post,data,status_code,expect):
    ...
    def test_do_all_attendance(self,url,post,data,status_code,expect):
