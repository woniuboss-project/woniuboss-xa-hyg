...
em_employment_info = Utility.get_excel_to_tuple(test_config_info[3])
em_technology_interview_info = Utility.get_excel_to_tuple(test_config_info[4])
...
class EmploymentManagementTest(unittest.TestCase):
	...
	def test_fm_end_time(cls, data, expect):
	...
	def test_em_interview_query(cls, technology, expect):
		...
		actual = Service.text_css(cls.driver,'#stuInfo_table > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(1)')
		cls.assertEqual(actual, expect)
		cls.driver.refresh()
	...
	def test_em_class_query(cls, class_query, expect):
		...
		actual = Service.text_css(cls.driver,'#stuInfo_table1 > tbody:nth-child(2) > tr:nth-child(1) > td:nth-child(1)')
		cls.assertEqual(actual, expect)
		cls.driver.refresh()
	...
	def test_em_submit(cls, data, expect):
		...
		em_results = Service.text_css(cls.driver,'stuInfo_table1 > tbody:nth-child(2) > tr:nth-child(1) > '
												 'td:nth-child(8) > label:nth-child(2)')
		if em_results == '电脑回收':
			actual = 'EM_correct'
		else:
			actual = 'EM_failed'
