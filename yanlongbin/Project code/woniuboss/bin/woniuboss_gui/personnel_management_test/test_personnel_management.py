...
# pm_department_info = Utility.get_excel_to_tuple(test_config_info[1])
...
pm_new_input_info = Utility.get_excel_to_tuple(test_config_info[7])
pm_modify_input_info = Utility.get_excel_to_tuple(test_config_info[7])
...
class PersonnelManagementTest(unittest.TestCase):
    ...
    # # 界面
    # @parameterized.expand(pm_department_info)
    # def test_m_recipients_interface(cls, data, expect):
    # 	PersonnelManagement(cls.driver).pm_department_interface()
    # 	if Service.is_element_present(cls.driver, By.LINK_TEXT, '新增'):
    # 		actual = 'PM_correct'
    # 	else:
    # 		actual = 'PM_failed'
    # 		cls.driver.refresh()
